# Claude / Codex Tools — e-Tax Invoice Project

ภาพรวมของ agents, skills, commands, และ MCP servers ทั้งหมดที่ติดตั้งไว้ในโปรเจกต์นี้. ดู parity checklist ได้ที่ `docs/agents/tool-parity.md`.
...
These are mirrored into both `.claude/skills/` and `.agents/skills/` so Claude and Codex can use the same project playbook.
...
**Location convention**: Each skill is `.claude/skills/<name>/SKILL.md` and `.agents/skills/<name>/SKILL.md` with optional `scripts/`, `templates/`, `examples/` subdirs. Keep both trees aligned when adding a new project skill.
...
cp -R .claude/skills/my-new-skill .agents/skills/my-new-skill
```
...
In Codex, use the currently exposed tool list. A server can be present in `.mcp.json` but unavailable in a specific Codex session if the runtime did not expose it. Serena is configured at user level for Codex, not inside `.mcp.json`.
