...
- RD VAT open-data background sync is deployed:
  ...
  - MOC/DBD English-name enrichment foundation is implemented:
    ...
    - Render deploy workflow now waits for the separate GitHub `Typecheck` workflow for the same commit before triggering Render, then prefers `RENDER_DEPLOY_HOOK_URL` and only falls back to Render API deploy if no hook is configured. Earlier API-triggered deploys `25782571599`, `25782852178`, `25782959201`, and `25783066255`, plus hook deploys `25783484199`, `25783713949`, and `25783855109`, failed at Render `build_failed` with `failureReason:null` and no build logs, likely because Render was triggered while commit checks were still pending.
