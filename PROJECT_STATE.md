...
Last updated: 2026-05-13 12:55 Asia/Bangkok
...
- RD VAT open-data background sync is deployed:
  ...
  - MOC/DBD English-name enrichment foundation is implemented:
    ...
    - Pending local change: Render deploy workflow now requests `clearCache:"clear"` because deploy `25782571599` failed at Render `build_failed` while local backend/frontend builds and GitHub typecheck passed, suggesting stale/corrupt Render build cache.
