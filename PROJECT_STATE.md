...
- RD VAT open-data background sync is deployed:
  ...
  - MOC/DBD English-name enrichment foundation is implemented:
    ...
    - The manual deploy workflow still patches the Render API service to `autoDeploy=no` before creating a manual deploy, so use it only as an emergency deploy path.
