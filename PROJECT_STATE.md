...
- RD VAT open-data background sync is deployed:
  ...
  - MOC/DBD English-name enrichment foundation is implemented:
    ...
    - Render deploy workflow now waits for the separate GitHub `Typecheck` workflow for the same commit before triggering Render, then prefers `RENDER_DEPLOY_HOOK_URL` and only falls back to Render API deploy if no hook is configured. Earlier API-triggered deploys `25782571599`, `25782852178`, `25782959201`, and `25783066255`, plus hook deploys `25783484199`, `25783713949`, `25783855109`, and `25784003604`, failed at Render `build_failed` with `failureReason:null` and no build logs for the deploy id. Local backend/frontend builds and GitHub Typecheck still pass, so the remaining blocker is Render-side service/deploy configuration or dashboard-only build diagnostics.
    - `render.yaml` now sets `autoDeploy: false` for both `etax-invoice-api` and `etax-invoice-worker` so Render Blueprint auto-deploy does not race the GitHub Actions deploy hook/checks.
    - Deploy workflow now patches the Render API service to `autoDeploy=no` via Render API before creating a manual deploy, so the dashboard setting is corrected even if Blueprint sync has not applied yet.
