...
- RD VAT open-data background sync is deployed:
  ...
  - MOC/DBD English-name enrichment foundation is implemented:
    ...
    - Render deploy workflow now prefers `RENDER_DEPLOY_HOOK_URL` and only falls back to Render API deploy if no hook is configured; API-triggered deploys `25782571599`, `25782852178`, `25782959201`, and `25783066255`, plus hook deploys `25783484199` and `25783713949`, failed at Render `build_failed` with `failureReason:null` while local backend/frontend builds and GitHub typecheck passed. The workflow now queries Render build logs by deploy id (`taskRun`) after a failed deploy.
