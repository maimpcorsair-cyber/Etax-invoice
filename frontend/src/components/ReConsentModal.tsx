    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
        <h2 className="text-lg font-semibold text-slate-900">
          {t('reConsent.title', { defaultValue: isThai ? 'อัปเดตข้อกำหนดและนโยบาย' : 'Terms and policy update' })}
        </h2>
        ...
        <div className="mt-4 space-y-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
          <label className="flex items-start gap-3 text-sm text-slate-800 cursor-pointer">
            ...
            />
            <span>
              {t('signup.consent.legal', { defaultValue: isThai ? 'ฉันได้อ่านและยอมรับ' : 'I have read and accept the' })}{' '}
        ...
        >
          {submitting
            ...
            : (isThai ? 'ยอมรับและใช้งานต่อ' : 'Accept and continue')}
        </button>
      </div>
    </div>
  );
