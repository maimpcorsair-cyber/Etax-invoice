...
import { channelMappingsRouter } from './routes/channelMappings';
...
app.get('/api/health/workers', async (_req, res) => {
  try {
    ...
    const [
      ...
      masterSheetQueue.getFailed(0, 0),
    ]);
    ...
    res.status(stuck ? 503 : 200).json({
      ...
      queues: {
        ...
        'master-sheet-sync': {
          ...
          failed: masterFailed,
          lastFailedReason: masterFailedJobs[0]?.failedReason?.slice(0, 240) ?? null,
        },
      },
...
app.use('/api/share', quotationSharePublicRouter);
...
app.use('/api/company-documents', authenticate, companyDocumentsRouter);
app.use('/api/channel-mappings', authenticate, channelMappingsRouter);
