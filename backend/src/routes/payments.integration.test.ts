...
test('payments API recalculates invoice payment state across add/list/delete flow', async (t) => {
  ...
  await t.test(
    'create fixture, add two payments, then delete one and verify invoice totals',
    async () => {
      try {
        ...
        const payments = await api<{ data: Array<{ id: string; amount: number }> }>(
          `/api/invoices/${invoice.id}/payments`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        ...
        if (invoiceId) {
          const id = invoiceId;
          await withSystemRlsContext(prisma, (tx) => tx.invoice.deleteMany({ where: { id } }), {
            role: 'test',
          });
        }
        if (customerId) {
